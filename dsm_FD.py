#FULL DROWSINESS BUILD
#To be submitted. It works and can be run with correct dataset directories but it's a bloated beast full of a bunch of unnecessary code that should be cut before submitting.
...
def torch_device(prefer: Optional[str]='auto') -> torch.device:
    ...
    # Select device: explicit 'cuda'/'mps' or 'auto'
    try:
...
# ////////////////////////////
# === Generic FER Data and Processing ===
# ////////////////////////////
...
class GenericFERDataset(CsvImageDataset):
    ...
    def __init__(self, csv_file: str, root_dir: Optional[str]=None, transform=None, class_map: Optional[dict]=None, has_header: bool=False, sep: str=',', num_classes: Optional[int]=None, enable_alignment: bool=True, crop_tight: bool=False, margin_range: Tuple[float, float]=(0.08, 0.12), target_size: Optional[int]=None):
        ...
        if self.enable_alignment:
            try:
                self.detector = YuNetDetector(model_path='face_detection_yunet.onnx', input_size=(320, 320), conf_threshold=0.6, nms_threshold=0.3, top_k=5000)
            except Exception:
                self.detector = None
...
def run_dmd_export(out_root: Optional[str]=None) -> None:
    ...
    face_stream_keys = [k.strip().lower() for k in 'rgb_face,driver_front,face,driver_face'.split(',') if k.strip()]

    try:
        yunet = YuNetDetector(model_path='face_detection_yunet.onnx', input_size=(320, 320), conf_threshold=0.6, nms_threshold=0.3, top_k=5000)
...
def build_loaders(cfg: TrainConfig, device: Optional[torch.device]=None):
    ...
    def maybe_prepare_affectnet() -> None:
        seven = False
        cache_dir = os.path.join(os.getcwd(), 'outputs', 'affectnet_cache')
...
def run_training():
    ...
    pretrain_train_loader, pretrain_val_loader, _, _ = build_loaders(cfg, device)
