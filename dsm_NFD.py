...
# Safe imports for metrics/plots
try:
    from sklearn.metrics import precision_recall_fscore_support as _prfs, confusion_matrix as _confusion_matrix
except Exception:
    _prfs = None
...
def _affectnet_clean(labels_csv: str, images_root: str, cache_dir: Optional[str]=None, seven_class: bool=False) -> Tuple[str, Dict[_Any, int], List[int], Dict[int, int]]:
    ...
    def label_to_id(val: _Any) -> Optional[int]:
        if val is None:
            return None
    ...
    # Save mapping JSON
    mapping_json: Dict[_Any, int] = name_to_id.copy()
...
class GenericFERDataset(CsvImageDataset):
    ...
    def __init__(self, csv_file: str, root_dir: Optional[str]=None, transform=None, class_map: Optional[dict]=None, has_header: bool=False, sep: str=',', num_classes: Optional[int]=None, enable_alignment: bool=True, crop_tight: bool=False, margin_range: Tuple[float, float]=(0.08, 0.12), target_size: Optional[int]=None):
        ...
        if key in _GENFER_CLEAN_CACHE:
            self.samples = _GENFER_CLEAN_CACHE[key]['samples']
        ...
        _GENFER_CLEAN_CACHE[key] = {'samples': self.samples, 'stats': (dropped_corrupt, dropped_occluded, dropped_blurry)}
...
class EfficientNetFeatureExtractor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        leye = self._pool_roi(z, cast(torch.Tensor, self._roi_leye))
        reye = self._pool_roi(z, cast(torch.Tensor, self._roi_reye))
        eyes_512 = 0.5 * (leye + reye)
        mouth_512 = self._pool_roi(z, cast(torch.Tensor, self._roi_mouth))
...
def build_loaders(cfg: TrainConfig, device: Optional[torch.device]=None):
    ...
    def _maybe_prepare_affectnet() -> None:
        try:
        ...
        if labels_csv:
            ...
            cleaned_csv, mapping, uniq, counts = _affectnet_clean(labels_csv, images_root, cache_dir=cache_dir, seven_class=seven)
            cfg.affectnet_csv = cleaned_csv
            # K consistent with mapping length
            K = 7 if seven else 8
            cfg.num_classes = K
            print(f'AffectNet: K={K} | unique={uniq} | counts={counts}')
    _maybe_prepare_affectnet()
    ...
    pretrain_val_ds = Subset(pretrain_val_base, cast(_Any, pretrain_val_indices).indices if hasattr(pretrain_val_indices, 'indices') else [])
    ...
    if cfg.dmd_csv:
        ...
        dmd_val_ds = Subset(dmd_val_full, cast(_Any, dmd_val_indices).indices if hasattr(dmd_val_indices, 'indices') else [])
            ...
            for i in ds.indices:
                ...
                if isinstance(base, ConcatDataset):
                    ...
                    if hasattr(base, 'samples'):
                        try:
                            lab = cast(_Any, base).samples[i][1]
                            lbls.append(lab)
                        except Exception:
                            try:
                                _x, lab = base[i]
...
def _autodiscover_defaults() -> dict:
    ...
    if p:
        info['rafdb_csv'] = p
    p = _maybe(os.path.join(os.getcwd(), 'datasets', 'affectNet', 'Train')) or _maybe(os.path.join(os.getcwd(), 'datasets', 'affectNet'))
    if p:
    ...
    dmd_export_root = _maybe(os.path.join(os.getcwd(), 'outputs', 'dmd_export'))
    if dmd_export_root:
        # Prefer frames CSV, else fallback to a simple CSV in export root
        for name in ['dmd_frames.csv', 'dmd.csv']:
            p = _maybe(os.path.join(dmd_export_root, name))
            if p:
                info['dmd_csv'] = p
                break
        ...
        for name in ['dmd_frames.csv', 'dmd_train.csv', 'dmd.csv']:
            p = _maybe(os.path.join(os.getcwd(), name))
            if p:
                info['dmd_csv'] = p
                break
...
def _make_default_cfg() -> TrainConfig:
    d = _autodiscover_defaults()
...
def run_training():
    ...
    def _peek(loader, name: str, max_batches: int=1):
        if loader is None:
            return
        try:
            ...
            pass
    if not skip_generic:
        _peek(pretrain_train_loader, 'pretrain_train', 1)
        _peek(pretrain_val_loader, 'pretrain_val', 1)
        if dmd_train_loader is not None:
            _peek(dmd_train_loader, 'dmd_train', 1)
    ...
    def _make_head(name: str='arcface') -> nn.Module:
        margin_init = 0.0 if safe_start else arcface_margin
        return ArcFaceHead(cast(int, cast(_Any, backbone).feature_dim), cfg.num_classes, m=float(margin_init), s=float(arcface_scale))
    head_name = 'arcface'
    ...
    def _read_prev_best(out_dir: str, preferred: str) -> float:
        try:
    ...
    def _param_stats(m: nn.Module) -> Tuple[int, int]:
        total = sum(p.numel() for p in m.parameters())
        trainable = sum(p.numel() for p in m.parameters() if p.requires_grad)
        return (int(total), int(trainable))
    try:
        b_tot, b_tr = _param_stats(backbone)
        h_tot, h_tr = _param_stats(head_m)
    ...
    def _sanity_ce_once(loader) -> None:
        try:
    ...
    if cfg.dmd_csv:
        ...
        def labels_from_subset(sub):
            ...
            if hasattr(base, 'samples'):
                for i in idxs:
                    try:
                        lbls.append(cast(_Any, base).samples[i][1])
                    except Exception:
                        pass
            else:
                for i in idxs:
        ...
        def _group_key_from_path(rel: str) -> str:
        ...
        num_classes_dmd = K
        head_ft = ArcFaceHead(cast(int, cast(_Any, backbone).feature_dim), num_classes_dmd).to(device)
        try:
        ...
        for epoch in range(3):
            tr_loss, tr_acc, _mf1, _pa, _la = run_epoch_ft(train_loader, True)
                ...
                try:
                    if _prfs is not None and _confusion_matrix is not None and preds_all is not None and labels_all is not None:
                        ...
                        for cid in range(Kloc):
                            print(f'{cid:8d} | {prec_arr[cid]:9.4f} | {rec_arr[cid]:6.4f} | {f1_arr[cid]:.4f} | {_safe_int(sup_arr[cid])}')
                        cm = _confusion_matrix(labels_all, preds_all, labels=list(range(Kloc)))
                        print('Confusion matrix (rows=true, cols=pred):')
                        print(cm)
                        if _plt is not None:
                            _plt.figure(figsize=(6,5))
                            _plt.imshow(cm, interpolation='nearest', cmap='Blues')
...
def run_gen_csv_from_folders(root_dir: str, out_csv: str, extensions: str='.jpg,.jpeg,.png,.bmp', save_class_map: Optional[str]=None, use_class_map: Optional[str]=None) -> None:
    exts = {e.lower().strip() for e in extensions.split(',') if e.strip()}
    class_map = load_class_map(use_class_map)
    _gen_csv_from_folders(root_dir, out_csv, exts, class_map, save_class_map)

def run_gen_csv_from_rafdb(root_dir: str, out_csv: str, split: str='train', image_subdir: Optional[str]=None) -> None:
    _gen_csv_from_rafdb(root_dir, out_csv, split=split, image_subdir=image_subdir)
...
def _gen_csv_from_affectnet_labels(labels_csv: str, images_root: str, out_csv: str, keep_classes: Iterable[int], map_unknown: Optional[int]=None, out_map_json: Optional[str]='affectnet_map.json') -> int:
    ...
    # filter rows
    def _keep(lbl: int) -> bool:
        if lbl < 0:
            return False
        if lbl in keep_set:
            return True
        return (map_unknown is not None) and (lbl == map_unknown)
    df = df[df['_label_raw'].map(lambda x: _keep(int(x)))].copy()
...
def run_export_feature_extractor(out_path: str, load_backbone_ckpt: Optional[str]=None) -> None:
    device = torch_device()
    backbone = EfficientNetFeatureExtractor('b0', pretrained=True).to(device)
    if load_backbone_ckpt and os.path.exists(load_backbone_ckpt):
        ckpt = torch.load(load_backbone_ckpt, map_location=device)
        state = None
        if isinstance(ckpt, dict):
            state = ckpt.get('state_dict') or ckpt.get('backbone_state')
        if isinstance(state, dict):
            backbone.load_state_dict(state, strict=False)
    os.makedirs(os.path.dirname(os.path.abspath(out_path)) or '.', exist_ok=True)
    torch.save({'type': 'efficientnet_backbone', 'variant': 'b0', 'state_dict': backbone.state_dict(), 'feature_dim': backbone.feature_dim}, out_path)
    print(f'Exported feature extractor to: {out_path}')

def run_extract_features(csv_path: str, root_dir: Optional[str], out_npz: str, class_map_json: Optional[str]=None) -> None:
    device = torch_device()
    backbone = EfficientNetFeatureExtractor('b0', pretrained=True).to(device)
    _, val_tf = make_transforms(112)
    class_map = load_class_map(class_map_json)
    ds = GenericFERDataset(csv_path, root_dir, transform=val_tf, class_map=class_map, num_classes=7, enable_alignment=True)
    num_workers = get_num_workers()
    pin_mem = isinstance(device, torch.device) and getattr(device, 'type', 'cpu') == 'cuda'
    dl = DataLoader(ds, batch_size=64, shuffle=False, num_workers=num_workers, pin_memory=pin_mem)
    backbone.eval()
    feats_list: List[np.ndarray] = []
    labels_list: List[int] = []
    paths_list: List[str] = [p for p, _ in ds.samples]
    with torch.no_grad():
        for images, labels in progress(dl, total=len(dl), unit='batch', desc='extract', leave=False):
            images = images.to(device, non_blocking=True)
            feats = backbone(images)
            feats = feats.detach().cpu().numpy()
            feats_list.append(feats)
            labels_list.extend(labels.detach().cpu().numpy().tolist())
    feats_all = np.concatenate(feats_list, axis=0) if feats_list else np.zeros((0, cast(int, cast(_Any, backbone).feature_dim)), dtype=np.float32)
...
def run_finetune_dmd(backbone_ckpt: str, dmd_csv: str, dmd_root: Optional[str], class_map_json: Optional[str], num_classes: int, epochs_head: int=3, epochs_ft: int=12, lr_head: float=1e-4, lr_backbone: float=1e-5) -> None:
    # Fine-tune flow: load pretrain backbone, new head for vigilance classes, freeze→unfreeze schedule
    seed_everything(42)
    device = torch_device()
    img_size = 112
    batch_size = 64
    train_tf, val_tf = make_transforms(img_size)
    class_map = load_class_map(class_map_json)
    # Build dataset and perform group-wise split (no leakage by video/session)
    ds_full = GenericFERDataset(dmd_csv, dmd_root, transform=train_tf, class_map=class_map, num_classes=num_classes, enable_alignment=True)
    total = len(ds_full)
    train_n = 0
    val_n = 0
    if total <= 1:
        train_indices, val_indices = list(range(total)), []
    else:
        desired = max(200, int(0.1 * total))
        val_n = min(total - 1, max(1, desired))
        val_ratio = float(val_n) / float(total)
        # derive group id from path
        def _group_key_from_path(rel: str) -> str:
            rel = str(rel or '').replace('\\', '/')
            parts = [p for p in rel.split('/') if p]
            if len(parts) >= 3:
    ...
    # Remap labels to contiguous 0..K-1 based on union labels
    labs_union = sorted(set(int(ds_full.samples[i][1]) for i in train_indices + val_indices))
    class_to_idx = {cls: i for i, cls in enumerate(labs_union)}
    for i in (train_indices + val_indices):
        rp, rl = ds_full.samples[i]
    ...
    K = len(labs_union)
    print(f"[DMD] classes={K} unique={labs_union}")
    train_ds, val_ds = Subset(ds_full, train_indices), Subset(ds_full, val_indices)
    ...
    def labels_from_subset(sub):
        lbls = []
        base = sub.dataset
        if isinstance(base, CsvImageDataset):
    ...
    num_workers = get_num_workers()
    pin_mem = isinstance(device, torch.device) and getattr(device, 'type', 'cpu') == 'cuda'
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=(sampler is None), sampler=sampler, num_workers=num_workers, pin_memory=pin_mem, drop_last=False)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_mem, drop_last=False)
    # Detailed DMD fine-tune loader summary (standalone fine-tune)
    try:
        tr_labels = labels_from_subset(train_ds)
        va_labels = labels_from_subset(val_ds)
        print("\n[DMD] Dataset summary:")
        print(f"  total={total}  train_n={train_n}  val_n={val_n}")
        print(f"  train label hist: {dict(Counter(tr_labels))}")
        print(f"  val   label hist: {dict(Counter(va_labels))}")
        print(f"  batch_size={batch_size}  train_batches={len(train_loader)}  val_batches={len(val_loader)}")
    except Exception:
        pass
    # Model
    backbone = EfficientNetFeatureExtractor('b0', pretrained=False).to(device)
    # load backbone weights only
    ckpt = torch.load(backbone_ckpt, map_location=device)
    bstate = ckpt.get('backbone_state') or ckpt.get('state_dict')
    if isinstance(bstate, dict):
        backbone.load_state_dict(bstate, strict=False)
    head_m = ArcFaceHead(cast(int, cast(_Any, backbone).feature_dim), K)
    head_m = head_m.to(device)
    try:
        object.__setattr__(head_m, 'out_features', int(K))
    except Exception:
        pass
    assert getattr(head_m, 'out_features', head_m.W.shape[0]) == K
    criterion = nn.CrossEntropyLoss(label_smoothing=0.05)
    out_dir = './outputs'
    os.makedirs(out_dir, exist_ok=True)
    best_acc = 0.0
    ...
    for p in backbone.parameters():
        p.requires_grad = False
    opt_head = torch.optim.AdamW(head_m.parameters(), lr=lr_head, weight_decay=1e-4)
    dmd_eval_banner_done = False
    def run_epoch(loader, train_mode: bool) -> Tuple[float, float]:
        nonlocal dmd_eval_banner_done
        if train_mode:
            backbone.train(); head_m.train()
        else:
            backbone.eval(); head_m.eval()
        ...
        for images, labels in progress(loader, total=len(loader), unit='batch', desc=('train' if train_mode else 'val'), leave=True):
            ...
            with torch.set_grad_enabled(train_mode):
                feats = backbone(images)
                if train_mode:
                    logits = head_m(feats, labels)
                    loss = criterion(logits, labels)
                else:
                    logits = head_m(feats, None)
                    if not dmd_eval_banner_done:
                        try:
                            print(f"[DMD][eval] using_label_free_logits=True  K={K}  head_out_features={getattr(head_m,'out_features', head_m.W.shape[0])}")
                        except Exception:
                            pass
                        dmd_eval_banner_done = True
                    loss = criterion(logits, labels)
            ...
            total += int(images.size(0))
        return (total_loss / max(1, total), total_correct / max(1, total))
    # Val metrics with macro-F1
    def _val_metrics(loader) -> Tuple[float, float, float, List[int], List[int]]:
        head_m.eval(); backbone.eval()
        tot_loss = 0.0; tot_correct = 0; tot = 0
        preds_all: List[int] = []; labels_all: List[int] = []
        for images, labels in loader:
            images = images.to(device, non_blocking=True)
            labels = labels.to(device, non_blocking=True)
            with torch.no_grad():
                ...
                logits_pred = head_m(feats, None)
                preds = torch.argmax(logits_pred, dim=1)
            tot_loss += float(loss.detach().cpu().item()) * images.size(0)
            tot_correct += int((preds == labels).sum().detach().cpu().item())
            tot += int(images.size(0))
            preds_all.extend(preds.detach().cpu().tolist())
            labels_all.extend(labels.detach().cpu().tolist())
        # macro F1
        Kloc = int(num_classes)
        f1s = []
        for c in range(Kloc):
            tp = sum(1 for p, t in zip(preds_all, labels_all) if p == c and t == c)
            fp = sum(1 for p, t in zip(preds_all, labels_all) if p == c and t != c)
            ...
            f1s.append(f1)
        macro_f1 = float(sum(f1s) / max(1, len(f1s)))
        return (tot_loss / max(1, tot), tot_correct / max(1, tot), macro_f1, preds_all, labels_all)

    for epoch in range(epochs_head):
        tr_loss, tr_acc = run_epoch(train_loader, True)
        va_loss, va_acc, va_f1, _preds_all, _labels_all = _val_metrics(val_loader)
        try:
            print(f"[DMD][HeadOnly] Epoch {epoch + 1}/{epochs_head}  train loss {tr_loss:.4f} acc {tr_acc:.4f} | val loss {va_loss:.4f} acc {va_acc:.4f} macroF1 {va_f1:.4f} | batches train={len(train_loader)} val={len(val_loader)}")
            # Extended evaluation report
            try:
                if _prfs is not None and _confusion_matrix is not None:
                    prec_arr, rec_arr, f1_arr, sup_arr = _prfs(_labels_all, _preds_all, labels=list(range(int(num_classes))), zero_division=0)
                    prec_arr = _ensure_list_metric(prec_arr); rec_arr = _ensure_list_metric(rec_arr); f1_arr = _ensure_list_metric(f1_arr); sup_arr = _ensure_list_metric(sup_arr)
                    print('class_id | precision | recall | f1 | support')
                    for cid in range(int(num_classes)):
                        print(f'{cid:8d} | {prec_arr[cid]:9.4f} | {rec_arr[cid]:6.4f} | {f1_arr[cid]:.4f} | {_safe_int(sup_arr[cid])}')
                    cm = _confusion_matrix(_labels_all, _preds_all, labels=list(range(int(num_classes))))
                    print('Confusion matrix (rows=true, cols=pred):')
                    print(cm)
                    if _plt is not None:
                        _plt.figure(figsize=(6,5))
        ...
        torch.save({'head_type': 'arcface', 'epoch': epoch + 1, 'phase': 'head_only', 'best_acc': best_acc, 'backbone_state': backbone.state_dict(), 'head_state': head_m.state_dict(), 'eff_variant': 'b0', 'feature_dim': backbone.feature_dim, 'num_classes': int(num_classes), 'class_map': class_map, 'preprocess': {'img_size': int(img_size), 'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225]}}, os.path.join(out_dir, 'last_finetune_arcface.pt'))
    # Phase B: unfreeze backbone
    for p in backbone.parameters():
        p.requires_grad = True
    optimizer = torch.optim.AdamW([
        {'params': backbone.parameters(), 'lr': lr_backbone, 'weight_decay': 1e-4},
        {'params': head_m.parameters(), 'lr': lr_head, 'weight_decay': 1e-4},
    ])
    for epoch in range(epochs_ft):
        # reuse run_epoch with optimizer
        backbone.train(); head_m.train()
        total_loss = 0.0; total_correct = 0; total = 0
        for images, labels in progress(train_loader, total=len(train_loader), unit='batch', desc='train', leave=True):
            images = images.to(device, non_blocking=True); labels = labels.to(device, non_blocking=True)
            ...
            # label-free logits for prediction/metrics
            logits_pred = head_m(feats, None)
            preds = torch.argmax(logits_pred, dim=1); total_correct += int((preds == labels).sum().detach().cpu().item()); total += int(images.size(0))
        va_loss, va_acc, va_f1, _preds_all_u, _labels_all_u = _val_metrics(val_loader)
        try:
            print(f"[DMD][Unfreeze] Epoch {epoch + 1}/{epochs_ft}  train loss {total_loss/max(1,total):.4f} acc {total_correct/max(1,total):.4f} | val loss {va_loss:.4f} acc {va_acc:.4f} macroF1 {va_f1:.4f} | batches train={len(train_loader)} val={len(val_loader)}")
            # Extended evaluation report
            try:
                if _prfs is not None and _confusion_matrix is not None:
                    prec_arr, rec_arr, f1_arr, sup_arr = _prfs(_labels_all_u, _preds_all_u, labels=list(range(int(num_classes))), zero_division=0)
                    prec_arr = _ensure_list_metric(prec_arr); rec_arr = _ensure_list_metric(rec_arr); f1_arr = _ensure_list_metric(f1_arr); sup_arr = _ensure_list_metric(sup_arr)
                    print('class_id | precision | recall | f1 | support')
                    for cid in range(int(num_classes)):
                        print(f'{cid:8d} | {prec_arr[cid]:9.4f} | {rec_arr[cid]:6.4f} | {f1_arr[cid]:.4f} | {_safe_int(sup_arr[cid])}')
                    cm = _confusion_matrix(_labels_all_u, _preds_all_u, labels=list(range(int(num_classes))))
                    print('Confusion matrix (rows=true, cols=pred):')
                    print(cm)
                    if _plt is not None:
                        _plt.figure(figsize=(6,5))
                        ...
                        _plt.tight_layout()
                        os.makedirs('./outputs', exist_ok=True)
                        img_path = os.path.join('./outputs', f'confusion_matrix_epoch{epochs_head + epoch + 1}.png')
                        _plt.savefig(img_path)
                        _plt.close()
            except Exception:
                pass
        ...
        last_path = os.path.join(out_dir, 'last_finetune_arcface.pt')
        best_path = os.path.join(out_dir, 'best_finetune_arcface.pt')
        torch.save({'head_type': 'arcface', 'epoch': epochs_head + epoch + 1, 'best_acc': best_acc, 'backbone_state': backbone.state_dict(), 'head_state': head_m.state_dict(), 'eff_variant': 'b0', 'feature_dim': backbone.feature_dim, 'optimizer': optimizer.state_dict(), 'num_classes': int(num_classes), 'class_map': class_map, 'preprocess': {'img_size': int(img_size), 'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225]}}, last_path)
        if va_acc > best_acc:
            best_acc = va_acc
            torch.save({'head_type': 'arcface', 'epoch': epochs_head + epoch + 1, 'best_acc': best_acc, 'backbone_state': backbone.state_dict(), 'head_state': head_m.state_dict(), 'eff_variant': 'b0', 'feature_dim': backbone.feature_dim, 'num_classes': int(num_classes), 'class_map': class_map, 'preprocess': {'img_size': int(img_size), 'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225]}}, best_path)
    print(f'Fine-tune complete. Best val acc {best_acc:.4f}. Checkpoints saved to {out_dir}')

def run_export_aligned_crops(out_root: str, export_csv_name: str='aligned_metadata.csv') -> None:
    seed_everything(42)
    device = torch_device()
    ...
    try:
        ...
        yunet = None
        print(f'Warning: YuNet unavailable ({e}); exporting center-cropped resized images without landmarks.')
        ...
        if yunet is not None:
            try:
                ...
                if dets:
                    ...
                    pts5 = np.array(best['kps'], dtype=np.float32)
                    if pts5.shape == (5, 2):
                        aligned, pts_t = align_face_5pt(pil, pts5, out_size=cfg.img_size, margin=0.1)
    ...
    if cfg.affectnet_csv:
        pretrain_datasets.append(GenericFERDataset(cfg.affectnet_csv, cfg.affectnet_root, transform=train_tf, class_map=class_map, num_classes=cfg.num_classes, enable_alignment=True, crop_tight=True, margin_range=(0.08, 0.12), target_size=cfg.img_size))
    if cfg.rafdb_csv:
        pretrain_datasets.append(GenericFERDataset(cfg.rafdb_csv, cfg.rafdb_root, transform=train_tf, class_map=class_map, num_classes=cfg.num_classes, enable_alignment=True, crop_tight=True, margin_range=(0.08, 0.12), target_size=cfg.img_size))
    ...
    def subset_samples(sub) -> List[Tuple[str, int]]:
        ...
        for i in sub.indices:
            ...
            local = i - cum[di]
            d = datasets[di]
            path, label = cast(_Any, d).samples[local]
        ...
        for path, label in progress(samples, total=len(samples), unit='img', desc=f'export {tag}', leave=False):
            try:
                with Image.open(path) as im:
                    ...
                    out_path = os.path.join(class_dir, os.path.basename(path))
                    try:
                    ...
                    train_sum += flat.sum(axis=0).astype(np.float64)
                    train_sumsq += (flat ** 2).sum(axis=0).astype(np.float64)
                    train_count += arr.shape[0] * arr.shape[1]
                    rel = os.path.relpath(out_path, out_root).replace('\\', '/')
                    rows.append((rel, label, tag, prob, bm, path, le, reye, mo))
            except Exception:
                continue
    export_split(subset_samples(train_subset), 'train')
    export_split(subset_samples(val_subset), 'val')
    d = _autodiscover_defaults()
    for name in ('affectnet_eval_csv', 'rafdb_eval_csv'):
        p = d.get(name)
        if p:
            root = d.get('affectnet_eval_root') if 'affectnet' in name else d.get('rafdb_eval_root')
            ds_eval = GenericFERDataset(p, root, transform=val_tf, class_map=class_map, num_classes=cfg.num_classes, enable_alignment=True, crop_tight=True, margin_range=(0.08, 0.12), target_size=cfg.img_size)
            export_split(ds_eval.samples, 'test')
    csv_path = os.path.join(out_root, export_csv_name)
    ...
    with open(os.path.join(out_root, 'normalization_stats.json'), 'w', encoding='utf-8') as fjs:
        json.dump({'img_size': cfg.img_size, 'channels': 'rgb', 'computed_on': 'all', 'count_pixels_per_channel': int(train_count), 'mean': mean, 'std': std}, fjs, indent=2)

def run_smooth_frames_csv(in_path: str, out_path: str, method: str='majority', window_s: float=0.5, alpha: float=0.2, fps: Optional[float]=None) -> None:
    if pd is None:
    ...
    for col in ['session', 'frame_idx', 'label']:
    ...
    if 'time_sec' in df.columns:
        ...
        fps_val = fps
        if fps_val is None and 'fps' in df.columns and (len(df) > 0):
            try:
        ...
        if fps_val is None:
            fps_val = 30.0
        df['time_sec'] = df['frame_idx'].astype(float) / float(fps_val)
    method = method.lower()
    if method not in ('majority', 'ema'):
        raise ValueError("method must be 'majority' or 'ema'")

    def smooth_session(group: 'pd.DataFrame') -> 'pd.DataFrame':
        arr = group['label'].astype(int).to_numpy()
        times = group['time_sec'].astype(float).to_numpy()
        if method == 'majority':
            out = []
            for t in times:
                lo = t - window_s / 2
                ...
                if not np.any(mask):
                    ...
                    vals, cts = np.unique(arr[mask], return_counts=True)
                    out.append(int(vals[int(np.argmax(cts))]))
            group = group.copy()
            group['label_smooth'] = np.array(out, dtype=np.int64)
            return group
        else:
            out = []
            ema = float(arr[0])
            for v in arr:
                ...
                out.append(int(round(ema)))
            group = group.copy()
            group['label_smooth'] = np.array(out, dtype=np.int64)
            return group
    if 'session' not in df.columns:
        raise ValueError("CSV must contain 'session' column to smooth per-session")
    df_out = df.groupby('session', as_index=False).apply(smooth_session).reset_index(drop=True)
    df_out.to_csv(out_path, index=False)
    print(f'Wrote smoothed CSV: {out_path}')


def run_dmd_export(out_root: Optional[str]=None) -> None:
    # New DMD export: folder-based pairing + OpenLABEL intervals -> per-frame labels
    return _run_dmd_export_new(out_root)

def _run_dmd_export_new(out_root: Optional[str]=None) -> None:
    seed_everything(42)
    ...
    def _list_files_ci(folder: str, exts: Iterable[str]) -> List[str]:
        out: List[str] = []
        try:
    ...
    def _json_face_video(json_obj: dict, folder: str) -> Optional[str]:
        try:
            ...
            if not uris:
                return None
            vids = _list_files_ci(folder, video_exts)
            vids_by_norm = { _norm_name(x): x for x in vids }
            for uri in uris:
    ...
    def _infer_split_from(json_obj: Optional[dict], folder: str) -> str:
        sp = None
        try:
    ...
    for root, _dirs, files in os.walk(dmd_root):
        try:
            if isinstance(_dirs, list):
                _dirs[:] = [d for d in _dirs if str(d).lower() != 'backup']
        except Exception:
            pass
    ...
    for folder in sorted(all_folders, key=lambda p: p.lower()):
        vids = _list_files_ci(folder, video_exts_local)
        js = _list_files_ci(folder, {'.json'})
        ...
        for (vid_path, json_path) in pairs:
            try:
                ...
                continue
            split = _infer_split_from(meta, folder)
            ...
            intervals = _parse_openlabel_actions(meta, float(fps))
            if not intervals:
...
def main():
    ...
    if getattr(args, 'train', False):
        try:
            if getattr(args, 'affectnet_labels_csv', None):
                ...
                _gen_csv_from_affectnet_labels(labels_csv, images_root, out_csv, keep_classes=keep, map_unknown=None, out_map_json=os.path.join(os.getcwd(), 'affectnet_map.json'))
                if getattr(args, 'affectnet_labels_csv_test', None):
                    ...
                    if os.path.exists(labels_csv_t) and os.path.exists(images_root_t):
                        ...
                        _gen_csv_from_affectnet_labels(labels_csv_t, images_root_t, out_csv_t, keep_classes=keep, map_unknown=None, out_map_json=os.path.join(os.getcwd(), 'affectnet_map.json'))
        except Exception as e:
    ...
    if getattr(args, 'affectnet_labels_csv', None):
        ...
        if getattr(args, 'affectnet_labels_csv_test', None):
            ...
            if os.path.exists(labels_csv_t):
                try:
                    cleaned_csv_t, _, _, _ = _affectnet_clean(labels_csv_t, images_root, cache_dir=cache_dir, seven_class=seven)
    ...
    if args.train:
        ...
        def _make_head(in_dim: int, num_classes: int) -> nn.Module:
            return ArcFaceHead(in_dim, num_classes)
        head_name: str = 'arcface'

        def _find_latest_ckpt(out_dir: str, preferred: str) -> Optional[str]:
            try:
        ...
        best_acc = 0.0
        optimizer = None
        auto_ckpt_path = _find_latest_ckpt(cfg.out_dir, head_name)
        ckpt = None
        if auto_ckpt_path is not None:
            try:
                ...
                ckpt = None
        head = _make_head(cast(int, cast(_Any, backbone).feature_dim), cfg.num_classes).to(device)
        if isinstance(ckpt, dict):
